...
public class TestSpdxFileCollector {
	...
	public void setUp() throws Exception {
	    ...
	    AnyLicenseInfo concludedLicense = LicenseInfoFactory.parseSPDXLicenseString( DEFAULT_CONCLUDED_LICENSE, container );
	    ...
	    AnyLicenseInfo declaredLicense = LicenseInfoFactory.parseSPDXLicenseString( DEFAULT_DECLARED_LICENSE, container );
	    ...
	    this.spdxPackage = new SpdxPackage("TestPackage", concludedLicense, 
	                                       new AnyLicenseInfo[0], "Package copyright", 
	...
	public void testCollectFilesInDirectory() throws InvalidSPDXAnalysisException, SpdxCollectionException {
        ...
        for ( int i = 0; i < SpdxFiles.length; i++ ) {
            ...
            assertEquals( RelationshipType.GENERATES, 
                          SpdxFiles[i].getRelationships()[0].getRelationshipType() );
            assertEquals( spdxPackage.getName(), 
                          SpdxFiles[i].getRelationships()[0].getRelatedSpdxElement().getName() );
        }
	}
    ...
    {
        return singlePointerToString( range.getStartPointer() ) + ":" + 
                        singlePointerToString( range.getEndPointer() );
    }
